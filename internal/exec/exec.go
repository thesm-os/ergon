...
type Options struct {
	...
	//
	// Used by the branch gate to run `gobco` under
	// GODEBUG=gotypesalias=1; gobco's own go.mod declares `go 1.16`,
	// which predates generic type aliases, so without the override
	// it cannot type-check any package whose graph contains one.
	Env []string
